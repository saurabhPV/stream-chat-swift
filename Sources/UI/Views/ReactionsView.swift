...
final class ReactionsView: UIView {
    ...
    private func createEmojiView(reaction: Reaction, completion: @escaping Completion) -> UIView {
        ...
        var tapsCount = reaction.score
        var wasSend = false
        let maxCount = reaction.type.isRegular ? 1 : (reaction.type.maxCount - reaction.score)
        ...
        var tap = label.rx.tapGesture(configuration: { tap, delegate in
            ...
            .do(onNext: { [weak label] _ in
                tapsCount += 1
                tapsCount = min(tapsCount, maxCount)
        ...
        tap
            .subscribe(onNext: { [weak self] _ in
                ...
                if let add = completion(reaction.type, tapsCount) {
                    self?.updateLabel(reactionType: reaction.type, increment: add ? tapsCount : -1)
                }
                
                self?.dismiss()
                
                }, onDisposed: {
                    if !wasSend, tapsCount > 0 {
                        _ = completion(reaction.type, tapsCount)
                    }
            })
            .disposed(by: disposeBag)
